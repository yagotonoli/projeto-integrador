...
/**
 * Rota PUT /estoque/:id
 * Atualiza todos os campos do item (codigo_item, name, brand, category, quantity, lot) conforme enviados no corpo da requisição.
 * Todos os campos são obrigatórios para atualização.
 */
app.put('/estoque/:id', (req, res) => {
  const { id } = req.params;
  let { codigo_item, name, brand, category, quantity, lot } = req.body;

  // Se necessário, converte quantity para número
  quantity = parseInt(quantity);

  // Verifica se todos os campos foram informados
  if ([codigo_item, name, brand, category, quantity, lot].some(field => field === undefined || field === null)) {
    return res.status(400).send('Todos os campos são obrigatórios para atualização.');
  }

  const sql = 'UPDATE items SET codigo_item = ?, name = ?, brand = ?, category = ?, quantity = ?, lot = ? WHERE id = ?';
  db.query(sql, [codigo_item, name, brand, category, quantity, lot, id], (err, result) => {
    if (err) {
